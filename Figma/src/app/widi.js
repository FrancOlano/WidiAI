...
let _midiRaf = 0, _midiStartTs = 0, _midiStartTime = 0;
...
function injectCSS(container) {
  ...
  style.textContent = `
/* Base reset & layout */
.widi-app*{box-sizing:border-box;margin:0;padding:0;}
...
/* Layout */
.w-content{flex:1;overflow:hidden;display:flex;flex-direction:column;position:relative;z-index:1;}

/* Premium Glass Panels */
.w-panel{border-radius:20px;padding:20px;background:linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.015));border:1px solid rgba(255,255,255,0.1);backdrop-filter:blur(16px) saturate(160%);box-shadow:0 8px 32px rgba(0,0,0,0.35),0 1px 0 rgba(255,255,255,0.06) inset,0 -1px 0 rgba(0,0,0,0.2) inset;position:relative;}
...
/* Dashboard Layout */
.w-dashboard{display:flex;flex-direction:column;flex:1;padding:20px 24px 0;gap:14px;overflow:hidden;}
.w-top-grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;}
...
/* Waveform Container */
.w-waveform{width:100%;height:72px;border-radius:10px;overflow:hidden;background:rgba(0,0,0,0.4);border:1px solid rgba(255,255,255,0.06);box-shadow:0 2px 8px rgba(0,0,0,0.2) inset;}
...
const HARMONICS = [[1, 1], [2, .45], [3, .2], [4, .08], [5, .04], [6, .02]];
...
class PianoRoll {
  ...
  _stopNote(midi) {
    const g = this.activeGains.get(midi);
    if (!g || !this.audioCtx) return;
    const now = this.audioCtx.currentTime;
    try { g.gain.cancelScheduledValues(now); g.gain.setValueAtTime(g.gain.value, now); g.gain.exponentialRampToValueAtTime(0.0001, now + 0.12); } catch (_) {}
    this.activeGains.delete(midi);
  }

  _playNote(midi) {
    this._stopNote(midi);
    const ac = this._ac(), freq = this._freq(midi), now = ac.currentTime;
    const baseGain = 0.18 + (midi - MIDI_LO) / (MIDI_HI - MIDI_LO) * 0.08;
    const master = ac.createGain();
    master.gain.setValueAtTime(0, now);
    master.gain.linearRampToValueAtTime(baseGain, now + 0.006);
    master.connect(ac.destination);
    const lp = ac.createBiquadFilter();
    lp.type = 'lowpass'; lp.frequency.value = Math.min(8000, freq * 8); lp.Q.value = 0.3;
    lp.connect(master);
    const dec = 1.8 + (midi - MIDI_LO) / (MIDI_HI - MIDI_LO) * 1.2;
    HARMONICS.forEach(([r, a]) => {
      const osc = ac.createOscillator(), g = ac.createGain(), hd = dec / (r * 0.7);
      osc.type = 'sine'; osc.frequency.value = freq * r;
      g.gain.setValueAtTime(a, now); g.gain.exponentialRampToValueAtTime(0.0001, now + hd);
      osc.connect(g); g.connect(lp); osc.start(now); osc.stop(now + hd + 0.05);
    });
    this.activeGains.set(midi, master);
  }
  ...
  _bindEvents() {
    ...
    const release = m => { this.pressedKeys.delete(m); this._stopNote(m); };
    const releaseAll = () => { this.pressedKeys.forEach(m => this._stopNote(m)); this.pressedKeys.clear(); };
...
function destroyInstances() {
  ...
  cancelAnimationFrame(_midiRaf); state.midiPlaying = false;
}
...
function renderDashboard(content) {
      ...
      <div class="w-top-grid">
        ...
        <div class="w-panel">
          ...
          <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
            ...
            <div style="flex:1;min-width:0;">
              ...
              <button class="w-upload-btn" id="upload-btn">${ICON.upload(12,'#93c5fd')} <span>Upload .wav / .mp4</span></button>
              <input type="file" id="file-input" accept=".wav,.mp3,.mp4,.ogg,.flac,.aiff" style="display:none;">
            </div>
          </div>
        ...
        <div class="w-panel">
          ...
          <div style="margin-bottom:12px;">
            <input type="range" class="w-seek" id="midi-seek" min="0" max="${TOTAL_DURATION}" step="0.05" value="${state.midiTime}" ${state.stage!=='ready'?'disabled':''} style="background:linear-gradient(to right,#8b5cf6 ${(state.midiTime/TOTAL_DURATION)*100}%,rgba(255,255,255,0.1) ${(state.midiTime/TOTAL_DURATION)*100}%);">
            <div style="display:flex;justify-content:space-between;margin-top:4px;">
              <span style="font-size:10px;color:#6b7280;" id="midi-cur">${fmtTime(state.midiTime)}</span>
              <span style="font-size:10px;color:#6b7280;">${fmtTime(TOTAL_DURATION)}</span>
            </div>
          </div>
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:12px;">
            <div class="w-stat-box"><div style="color:#6b7280;margin-bottom:2px;">${ICON.music2(11,'#6b7280')}</div><p style="font-size:12px;color:#e5e7eb;font-weight:600;">${state.stage==='ready'?MIDI_NOTES.length:'—'}</p><p style="font-size:9px;color:#4b5563;">Notes</p></div>
            <div class="w-stat-box"><div style="color:#6b7280;margin-bottom:2px;">${ICON.clock(11,'#6b7280')}</div><p style="font-size:12px;color:#e5e7eb;font-weight:600;">${state.stage==='ready'?'0:48':'—'}</p><p style="font-size:9px;color:#4b5563;">Duration</p></div>
            <div class="w-stat-box"><div style="color:#6b7280;margin-bottom:2px;">${ICON.activity(11,'#6b7280')}</div><p style="font-size:12px;color:#e5e7eb;font-weight:600;">${state.stage==='ready'?'120 BPM':'—'}</p><p style="font-size:9px;color:#4b5563;">Tempo</p></div>
          </div>
  ...
  if (_waveform && state.stage !== 'idle') _waveform.update(WAVEFORM, state.midiTime, TOTAL_DURATION, state.isRecording);

  const pianoBody = content.querySelector('#piano-body');
  if (pianoBody) { _pianoRoll = new PianoRoll(pianoBody, MIDI_NOTES); _pianoRoll.setTime(state.midiTime); _pianoRoll.setPlaying(state.midiPlaying); }
  ...
  content.querySelector('#midi-seek').addEventListener('input', e => {
    ...
    if (_waveform) _waveform.update(WAVEFORM, state.midiTime, TOTAL_DURATION, state.isRecording);
    if (state.midiPlaying) { _midiStartTs = performance.now(); _midiStartTime = state.midiTime; }
    _updateSeek(content);
  });
}
...
function _updateSeek(content) {
  ...
  if (seek) { seek.value = state.midiTime; const p = (state.midiTime / TOTAL_DURATION) * 100; seek.style.background = `linear-gradient(to right,#8b5cf6 ${p}%,rgba(255,255,255,0.1) ${p}%)`; }
  ...
  if (_waveform) _waveform.update(state.stage !== 'idle' ? WAVEFORM : [], state.midiTime, TOTAL_DURATION, state.isRecording);
}

function _midiTick(content, ts) {
  if (!state.midiPlaying) return;
  const t = _midiStartTime + (ts - _midiStartTs) / 1000;
  if (t >= TOTAL_DURATION) { state.midiPlaying = false; state.midiTime = 0; renderDashboard(content); return; }
  state.midiTime = t;
  _updateSeek(content);
  _midiRaf = requestAnimationFrame(_midiTick.bind(null, content));
}

function _midiPlayPause(content) {
  if (state.stage !== 'ready') return;
  if (state.midiPlaying) { state.midiPlaying = false; cancelAnimationFrame(_midiRaf); }
  else { state.midiPlaying = true; _midiStartTs = performance.now(); _midiStartTime = state.midiTime; _midiRaf = requestAnimationFrame(_midiTick.bind(null, content)); }
...
function _midiStop(content) { state.midiPlaying = false; state.midiTime = 0; cancelAnimationFrame(_midiRaf); _updateSeek(content); const btn = content.querySelector('#midi-play'); if (btn) btn.innerHTML = ICON.play(20,'white'); }

function _downloadMidi() {
  if (state.stage !== 'ready') return;
  const blob = new Blob(['MIDI placeholder'], { type: 'audio/midi' });
  const url = URL.createObjectURL(blob), a = document.createElement('a');
  a.href = url; a.download = 'widi_output.mid'; a.click(); URL.revokeObjectURL(url);
}
...
async function _handleRecord() {
  ...
  if (state.isRecording) {
    ...
    try {
      ...
      _mediaRecorder = new MediaRecorder(stream); _audioChunks = [];
      ...
      _mediaRecorder.onstop = () => {
        ...
        state.audioUrl = _audioUrlRef; state.fileName = 'recording.wav'; state.stage = 'loaded';
        renderDashboard(content);
      };
      _mediaRecorder.start(); state.isRecording = true; state.stage = 'idle';
    } catch (_) {
      state.isRecording = true; state.stage = 'idle';
      _recTimer = setTimeout(() => { state.isRecording = false; state.stage = 'loaded'; state.fileName = 'piano_recording.wav'; renderDashboard(content); }, 7000);
    }
  }
  renderDashboard(content);
}
...
function _handleUpload(e) {
  ...
  if (_mediaRecorder && _mediaRecorder.state !== 'inactive') _mediaRecorder.stop();
  state.isRecording = false; if (_recTimer) clearTimeout(_recTimer);
  if (_audioUrlRef) URL.revokeObjectURL(_audioUrlRef);
  _audioUrlRef = URL.createObjectURL(file); state.audioUrl = _audioUrlRef; state.fileName = file.name; state.stage = 'loaded';
  e.target.value = '';
  renderDashboard(document.getElementById('w-content'));
}

function _handleConvert(content) {
  if (state.stage !== 'loaded') return;
  state.stage = 'processing'; state.progress = 0;
  renderDashboard(content);
  let p = 0;
  _progressTimer = setInterval(() => {
    p += Math.random() * 8 + 2;
    if (p >= 100) {
      p = 100; clearInterval(_progressTimer);
      setTimeout(() => { state.stage = 'ready'; state.progress = 0; state.midiTime = 0; renderDashboard(content); }, 400);
    }
    state.progress = Math.min(100, p);
    const bar = content.querySelector('#proc-bar'), pct = content.querySelector('#proc-pct');
    ...
    content.querySelectorAll('[data-step]').forEach((el, i) => {
      ...
      const dot = el.querySelector('div'), sp = el.querySelector('span');
