...
- Python 3.8 or newer
- `pip`
- Node.js (optional, used to serve the frontend locally)
- System dependencies: ffmpeg, portaudio, libsndfile
...
Note: macOS system dependencies are installed via Homebrew unless `--skip-system-deps` is set.

Common options:

- `--venv` / `-VenvDir`: virtual environment directory
- `--skip-system-deps`: skip installing FFmpeg and system libraries
- `--generate-frontend-env`: generate `frontend/static/env.js` from `.env.frontend`
- `--api-url` / `-ApiUrl`: backend URL to write into `.env.frontend` (used with `--generate-frontend-env`)
...
Open `http://localhost:8001/templates/index.html` and set the API URL to `http://localhost:8000` in the Settings panel.

Option B (Node static server):

```bash
npx serve frontend -l 8001
```
...
The backend will be available at `http://localhost:8000`. Serve the frontend separately (see above) and point it to the backend URL.
...
| `WIDI_ALLOWED_ORIGINS` | No | `http://localhost:8001,http://127.0.0.1:8001` | Comma-separated list of allowed frontend origins for CORS. |

### Frontend Runtime Configuration

Production deployments use `frontend/static/runtime-config.json` (generated by the GitHub Pages workflow) to set the API base URL at runtime.

For local development, create `.env.frontend` and generate `frontend/static/env.js` with `scripts/generate_frontend_env.py`, or set the API URL in the Settings panel at runtime.
...
See [docs/ai-context.md](docs/ai-context.md) for the detailed implementation context covering the frontend/backend decoupling plan, runtime config pipeline, installer requirements, and Docker targets.
